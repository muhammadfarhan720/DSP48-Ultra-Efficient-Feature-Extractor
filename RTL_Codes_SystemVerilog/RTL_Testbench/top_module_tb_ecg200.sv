...
//read the CSV to the declared data_buffer
  ...
  initial begin
    file = $fopen("quantized_input_train_ECG200.csv", "r");
    if (file == 0) begin
...
parameter IN_ROWS = 100;       // 100 input rows
parameter IN_COLS = 96;        // 96 timesteps per row
...
initial begin
    ...
    output_file = $fopen("ECG200_results.csv", "w");
    for(int r = 0; r < OUT_ROWS; r++) begin
        for(int c = 0; c < OUT_COLS; c++) begin
