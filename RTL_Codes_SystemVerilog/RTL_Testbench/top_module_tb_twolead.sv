...
//read the CSV to the declared data_buffer
  ...
  initial begin
    file = $fopen("quantized_input_train_ECGTwoLead.csv", "r");
    if (file == 0) begin
...
parameter IN_ROWS = 1139;       // 100 input rows
parameter IN_COLS = 82;        // 96 timesteps per row
...
initial begin
    ...
    output_file = $fopen("ECGTwolead_results.csv", "w");
    for(int r = 0; r < OUT_ROWS; r++) begin
        for(int c = 0; c < OUT_COLS; c++) begin
