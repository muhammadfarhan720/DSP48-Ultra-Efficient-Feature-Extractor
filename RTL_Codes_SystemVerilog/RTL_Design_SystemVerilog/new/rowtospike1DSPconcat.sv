module rowtospike1DSPconcat( 
    ...
    localparam signed [47:0] VTH = 48'sd10000000; // Adjust the value as needed
   ...
   // reg [3:0] cycle_counter = 4'b0000; // 4-bit counter for sequencing (12 cycles total)
    ...
    always @(posedge CLK) begin
        case (counter)
            4'b0001: begin CONCAT_sel <= X[0];  C_sel <= VINX[0]; end
            4'b0010: begin CONCAT_sel <= ~X[0] + 1; C_sel <= VINX[1]; end
            4'b0011: begin CONCAT_sel <= X[1];  C_sel <= VINX[2]; end
            4'b0100: begin CONCAT_sel <= ~X[1] + 1;  C_sel <= VINX[3]; end
        endcase
